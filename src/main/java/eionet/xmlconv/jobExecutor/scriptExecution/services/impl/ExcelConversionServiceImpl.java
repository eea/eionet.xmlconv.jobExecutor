...
@Service
public class ExcelConversionServiceImpl implements ExcelConversionService {
    ...
    private List<ExcelStyleService> styles = null;
     ...
     */
    @Autowired
    public ExcelConversionServiceImpl() {
        wb = new HSSFWorkbook();
    }
    ...
    public void addRow(String defStyle, String defType) {
        ...
        short idx = getStyleIdxByName(defStyle, ExcelStyleService.STYLE_FAMILY_TABLE_CELL);
    ...
    public void addCell(String type, String str_value, String style_name) {
        ...
        short idx = -1;
        if (style_name != null) {
            idx = getStyleIdxByName(style_name, ExcelStyleService.STYLE_FAMILY_TABLE_CELL);
        }
        ...
        if (currentRow == 0 && idx > -1) {
            ...
            if (columns.size() > currentCell) {
                ...
                ExcelStyleService definedStyle = getStyleByName(column_style_name, ExcelStyleService.STYLE_FAMILY_TABLE_CELL);
    ...
    @Override
    public void addStyle(ExcelStyleService style) {
        if (style == null) {
            return;
        }
        if (styles == null) {
            styles = new ArrayList<ExcelStyleService>();
        }

        if (!styleExists(style)) {
            styles.add(style);
        }
        if (style.getFamily().equals(ExcelStyleService.STYLE_FAMILY_TABLE_CELL)) {
     ...
     * @param style Style
     */
    private void addStyleToWorkbook(ExcelStyleService style) {
     ...
     */
    private boolean styleExists(ExcelStyleService style) {
        if (style == null) {
            return false;
        }
    ...
    @Override
    public ExcelStyleService getStyleByName(String name, String family) {
        ...
        for (int i = 0; i < styles.size(); i++) {
            ExcelStyleService style = styles.get(i);
    ...
    @Override
    public void writeToFile() throws ConversionException {
        ...
        try {
            ...
            throw new ConversionException("ErrorConversionHandler - couldn't save the Excel file: " + e.toString());
        } finally {
    ...
    @Override
    public void writeToFile(OutputStream outstream) throws ConversionException {
